...
class ScreepsWebpackPlugin {
  ...
  apply (compiler) {
    ...
    compiler.hooks.afterEmit.tapAsync(pluginName, (compilation, cb) => {
      ...
      .then((modules) => {
		// NOTE: Removed functionality to auto-upload scripts to screeps.com - Please see screeps-webpack-plugin for more details
        const {branch} = this.options
        compilation.hooks[BEFORE_COMMIT].call(branch, modules)
        return compilation.hooks[AFTER_COMMIT].call(null)
      })
      .then(cb)
      .catch((err) => {
